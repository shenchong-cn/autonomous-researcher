...
try:
    ...
    print("Warning: Recovery engine not available, using mock responses", file=sys.stderr)
...
if RECOVERY_ENGINE_AVAILABLE:
    try:
        recovery_engine = RecoveryEngine()
        print("Recovery engine initialized successfully", file=sys.stderr)
    except Exception as e:
        print(f"Failed to initialize recovery engine: {e}", file=sys.stderr)
        RECOVERY_ENGINE_AVAILABLE = False
...
def get_resumable_experiments() -> Dict[str, Any]:
    ...
    if not RECOVERY_ENGINE_AVAILABLE or not recovery_engine:
        # Fallback to direct file reading if recovery engine not available
        try:
                            ...
                            try:
                                with open(state_file, 'r', encoding='utf-8') as f:
                                    ...
                                    resumable.append({
                                        ...
                                        "created_at": state.get("created_at", state.get("updated_at", "")),
                                        "updated_at": state.get("updated_at", ""),
                                        "current_step": state.get("current_step", 0),
                                        "max_steps": state.get("max_steps", 0),
                                        "status": state.get("status", "hung"),
    ...
    try:
        # Use the recovery engine's state manager
        resumable_data = recovery_engine.state_manager.get_resumable_experiments()
        return {
            "resumable_experiments": resumable_data,
            "count": len(resumable_data)
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
def get_experiment_state(experiment_id: str) -> Dict[str, Any]:
    ...
    try:
        status = recovery_engine.get_experiment_status(experiment_id)
        if not status:
            raise HTTPException(status_code=404, detail="Experiment not found")
        return status
    except HTTPException:
...
def resume_experiment(req: ResumeRequest) -> Dict[str, Any]:
    ...
    if not RECOVERY_ENGINE_AVAILABLE or not recovery_engine:
        return {
            "success": False,
            "message": "Recovery engine not available",
            "checkpoint_used": req.checkpoint_id or "latest state"
        }

    try:
        print(f"[DEBUG] Attempting to resume experiment {req.experiment_id}", file=sys.stderr)
        # Call the real recovery engine
        success = recovery_engine.resume_execution(req.experiment_id, req.checkpoint_id)
        print(f"[DEBUG] Resume result: {success}", file=sys.stderr)

        if success:
            return {
                "success": True,
                "message": f"Experiment {req.experiment_id} resumed successfully",
                "checkpoint_used": req.checkpoint_id or "latest state"
            }
        else:
            return {
                "success": False,
                "message": f"Failed to resume experiment {req.experiment_id}",
                "checkpoint_used": req.checkpoint_id or "latest state"
            }
    except Exception as e:
        return {
            "success": False,
            "message": f"Error resuming experiment {req.experiment_id}: {str(e)}",
            "checkpoint_used": req.checkpoint_id or "latest state"
        }
...
def trigger_auto_recovery() -> Dict[str, Any]:
    ...
    try:
        recovered = recovery_engine.auto_recovery_check()
        return {
            "success": True,
            "recovered_experiments": recovered,
            "count": len(recovered),
            "message": f"Auto-recovered {len(recovered)} experiments"
        }
    except Exception as e:
        return {
            "success": False,
            "recovered_experiments": [],
            "count": 0,
            "message": f"Auto-recovery failed: {str(e)}"
        }
...
def delete_experiment_state(experiment_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Try to use recovery engine if available
        if RECOVERY_ENGINE_AVAILABLE and recovery_engine:
            # Try to use recovery engine's state manager
            try:
                state_manager = recovery_engine.state_manager
                state_file = Path(".orchestrator_state") / "experiments" / experiment_id / "state.json"
                experiment_dir = state_file.parent

                if experiment_dir.exists():
                    # Terminate any running processes for this experiment
                    recovery_engine._terminate_running_processes(experiment_id)
                    ...
                    return {
                        "success": True,
                        "message": f"Successfully deleted state for experiment {experiment_id}"
                    }
                else:
                    return {
                        "success": False,
                        "message": f"No state found for experiment {experiment_id}"
                    }
            except Exception as e:
                print(f"[DEBUG] Recovery engine deletion failed: {e}", file=sys.stderr)

        # Fallback: direct file system deletion
        experiment_dir = Path(".orchestrator_state") / "experiments" / experiment_id
        ...
        return {
            "success": False,
            "message": f"Failed to delete state for experiment {experiment_id}: {str(e)}"
        }
